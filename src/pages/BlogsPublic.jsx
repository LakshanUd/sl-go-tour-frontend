...
export default function BlogsPublic() {
  ...
  return (
    <>
      <Typography variant="h4" gutterBottom>Latest Blog Posts</Typography>
      {blogs.length === 0 ? (
              ...
              <Card sx={{ height: "100%", display: "flex", flexDirection: "column" }}>
                <CardActionArea onClick={() => openPost(b)}>
                  <CardMedia
                    component="img"
                    height="180"
                    image={b.image || "https://images.unsplash.com/photo-1496307042754-b4aa456c4a2d?q=80&w=1600&auto=format&fit=crop"}
                    alt={b.title}
                  />
                  <CardContent>
      ...
      <Dialog open={open} onClose={closePost} maxWidth="md" fullWidth>
        ...
        <DialogContent dividers>
          <Typography variant="subtitle2" color="text.secondary" sx={{ mb: 2 }}>
            By {active?.author} • {active?.publishedDate ? new Date(active.publishedDate).toLocaleString() : ""}
          </Typography>
          {active?.image && (
            <img
              src={active.image}
              alt={active.title}
              style={{ width: "100%", borderRadius: 12, marginBottom: 16, maxHeight: 420, objectFit: "cover" }}
            />
          )}
          <div
