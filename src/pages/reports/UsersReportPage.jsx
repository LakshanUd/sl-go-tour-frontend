// src/pages/reports/UsersReportPage.jsx
            ...
            <tbody>
              {data.map((item, idx) => (
                <tr key={idx} className="border-b border-neutral-100 hover:bg-neutral-50">
                  <td className="p-3 font-medium text-neutral-800">{item.name || item.country || item.role}</td>
                  <td className="p-3">{item.count || item.users || 0}</td>
                  <td className="p-3">{item.percentage || "—"}</td>
                  <td className="p-3">{item.lastActive || "—"}</td>
                  <td className="p-3">
    ...
    <div className="min-h-screen bg-neutral-50 pt-24">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        ...
        <div className="mb-8">
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-3xl font-bold text-neutral-900">User Reports</h1>
              <p className="text-neutral-600 mt-2">Analytics and insights for user demographics</p>
            </div>
            <div className="flex items-center gap-2">
              ...
              <button
                onClick={async () => {
                  try {
                    await generateUserReportPDF(reports);
                ...
                disabled={loading || !reports.topCountries?.length}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          ...
          <StatCard
            title="Growth Rate"
            value={`${Number(reports.growthRate || 0).toFixed(1)}%`}
            icon={BarChart3}
            color="red"
          />
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <ReportTable
            title="Top Countries by New Users"
            data={reports.topCountries || []}
            columns={["Country", "New Users", "Percentage", "Last Active", "Status"]}
          />

          {/* Moderators by Role */}
          <ReportTable
            title="Moderators by Role"
            data={reports.moderatorsByRole || []}
            columns={["Role", "Count", "Percentage", "Last Active", "Status"]}
          />

          {/* User Demographics */}
          <div className={`${CARD} p-6`}>
            <h3 className="text-lg font-semibold text-neutral-800 mb-4">User Demographics</h3>
            <div className="space-y-4">
              <div className="flex items-center justify-between p-3 bg-green-50 rounded-lg">
                <div className="flex items-center gap-3">
                  <Globe className="h-5 w-5 text-green-600" />
                  <span className="text-sm font-medium text-green-800">Top Region</span>
                </div>
                <span className="text-sm text-green-600">Asia Pacific</span>
              </div>
              <div className="flex items-center justify-between p-3 bg-blue-50 rounded-lg">
                <div className="flex items-center gap-3">
                  <Users className="h-5 w-5 text-blue-600" />
                  <span className="text-sm font-medium text-blue-800">Avg Age</span>
                </div>
                <span className="text-sm text-blue-600">28 years</span>
              </div>
              <div className="flex items-center justify-between p-3 bg-purple-50 rounded-lg">
                <div className="flex items-center gap-3">
                  <BarChart3 className="h-5 w-5 text-purple-600" />
                  <span className="text-sm font-medium text-purple-800">Gender Split</span>
                </div>
                <span className="text-sm text-purple-600">55% Female</span>
              </div>
            </div>
          </div>
          ...
          {/* User Engagement */}
          <div className={`${CARD} p-6`}>
            <h3 className="text-lg font-semibold text-neutral-800 mb-4">User Engagement</h3>
            <div className="space-y-4">
              <div className="flex items-center justify-between p-3 bg-green-50 rounded-lg">
                <div className="flex items-center gap-3">
              ...
              <div className="flex items-center justify-between p-3 bg-blue-50 rounded-lg">
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-sm font-medium text-blue-800">Avg Session</span>
                </div>
                <span className="text-sm text-blue-600">12 minutes</span>
              </div>
              <div className="flex items-center justify-between p-3 bg-purple-50 rounded-lg">
                <div className="flex items-center gap-3">
