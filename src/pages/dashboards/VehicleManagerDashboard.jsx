// src/pages/dashboards/VehicleManagerDashboard.jsx
...
export default function VehicleManagerDashboard() {
  ...
  useEffect(() => {
    const t = setTimeout(() => {
      setRecent([
        { id: "VH-3421", title: "Added vehicle: Toyota HiAce (12-seater)", when: "1h ago" },
        { id: "VH-3310", title: "Updated maintenance: Nissan Caravan", when: "Today" },
        { id: "VH-3285", title: "Updated fuel log: Kia Sorento", when: "Yesterday" },
      ]);
      setLoading(false);
    }, 450);
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
      ...
      <div className="mb-6 flex items-center justify-between">
        <h1 className="text-2xl font-semibold tracking-tight">Vehicle Manager</h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <section className="rounded-xl bg-white p-4 shadow-sm">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <div className="font-semibold text-neutral-900">Vehicle Manager</div>
                </div>
              </div>

              <div className="mt-4 grid grid-cols-3 gap-2 text-center">
                <MiniStat k="Fleet" v="128" />
                <MiniStat k="Available" v="96" />
                <MiniStat k="In Service" v="7" />
              </div>
        ...
        <main className="lg:col-span-2 space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <StatCard title="Vehicles available" value="96" icon={<Gauge className={`h-5 w-5 ${ICON_COLOR}`} />} />
            <StatCard title="Maintenance due (7d)" value="5" icon={<Wrench className={`h-5 w-5 ${ICON_COLOR}`} />} />
            <StatCard title="Avg. fuel economy" value="12.8 km/L" icon={<Fuel className={`h-5 w-5 ${ICON_COLOR}`} />} />
          </div>
          ...
          <div className={`rounded-xl p-[1px] ${GRAD_BG}`}>
            <section className="rounded-xl bg-white">
              ...
              <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-3 p-4">
                <DashLink
                  to="/admin/manage-vehicles"
                  icon={<Car className={`h-5 w-5 ${ICON_COLOR}`} />}
                  label="Manage Vehicles"
                />
                {/* Add more vehicle-only actions as needed */}
              </div>
            </section>
          </div>
