// src/pages/dashboards/AuthorDashboard.jsx
...
export default function AuthorDashboard() {
  ...
  useEffect(() => {
    const t = setTimeout(() => {
      setRecent([
        { id: "BL-284", title: "New draft created (10 Tips for Ella)", when: "45m ago" },
        { id: "BL-277", title: "Post published (Sigiriya Guide)", when: "Yesterday" },
        { id: "BL-269", title: "Cover image updated (Jaffna Food Tour)", when: "3 days ago" },
      ]);
      setLoading(false);
    }, 450);
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <aside className="lg:col-span-1 space-y-4">
          ...
          <div className={`rounded-xl p-[1px] ${GRAD_BG}`}>
            <section className="rounded-xl bg-white p-4 shadow-sm">
              ...
              <div className="mt-4 grid grid-cols-3 gap-2 text-center">
                <MiniStat k="Posts" v="128" />
                <MiniStat k="Drafts" v="6" />
                <MiniStat k="Published" v="122" />
              </div>
          ...
          <NavGroup title="Manage">
            <RailLink to="/admin/manage-blogs" icon={<PenLine className={`h-4 w-4 ${ICON_COLOR}`} />}>
              Blogs
            </RailLink>
          </NavGroup>
        </aside>
        ...
        <main className="lg:col-span-2 space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <StatCard title="Total posts" value="128" icon={<BookOpenText className={`h-5 w-5 ${ICON_COLOR}`} />} />
            <StatCard title="Drafts awaiting" value="6" icon={<FileText className={`h-5 w-5 ${ICON_COLOR}`} />} />
            <StatCard title="Featured posts" value="9" icon={<Sparkles className={`h-5 w-5 ${ICON_COLOR}`} />} />
          </div>

          {/* Quick actions — only blogs */}
          <div className={`rounded-xl p-[1px] ${GRAD_BG}`}>
            <section className="rounded-xl bg-white">
              ...
              <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-3 p-4">
                <DashLink
                  ...
                  icon={<PenLine className={`h-5 w-5 ${ICON_COLOR}`} />}
                  label="Manage Blogs"
                />
              </div>
            </section>
