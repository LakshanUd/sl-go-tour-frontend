// src/pages/dashboards/ComplaintFeedbackManagerDashboard.jsx
...
export default function ComplaintFeedbackManagerDashboard() {
  ...
  useEffect(() => {
    const t = setTimeout(() => {
      setRecent([
        { id: "CP-231", title: "Resolved complaint: Late Vehicle Pickup", when: "1h ago" },
        { id: "FB-884", title: "New feedback: Amazing tour guide!", when: "Today" },
        { id: "CP-227", title: "Complaint acknowledged: Room AC issue", when: "Yesterday" },
      ]);
      setLoading(false);
    }, 450);
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
      ...
      <div className="mb-6 flex items-center justify-between">
        <h1 className="text-2xl font-semibold tracking-tight">Complaint & Feedback Manager</h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <aside className="lg:col-span-1 space-y-4">
            ...
            <section className="rounded-xl bg-white p-4 shadow-sm">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <div className="font-semibold text-neutral-900">CF Manager</div>
                </div>
              </div>

              <div className="mt-4 grid grid-cols-3 gap-2 text-center">
                <MiniStat k="Complaints" v="128" />
                <MiniStat k="Feedbacks" v="342" />
                <MiniStat k="Resolved" v="91%" />
              </div>
          ...
          <NavGroup title="Manage">
            <RailLink
              ...
              icon={<MessageSquareWarning className={`h-4 w-4 ${ICON_COLOR}`} />}
            >
              Complaints
            </RailLink>
            <RailLink
              to="/admin/manage-feedbacks"
              icon={<ThumbsUp className={`h-4 w-4 ${ICON_COLOR}`} />}
            >
              Feedbacks
            </RailLink>
          </NavGroup>
        ...
        <main className="lg:col-span-2 space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <StatCard title="Open complaints" value="19" icon={<ShieldAlert className={`h-5 w-5 ${ICON_COLOR}`} />} />
            <StatCard title="Positive feedback" value="78%" icon={<Smile className={`h-5 w-5 ${ICON_COLOR}`} />} />
            <StatCard title="Negative feedback" value="6%" icon={<Frown className={`h-5 w-5 ${ICON_COLOR}`} />} />
          </div>
          ...
          <div className={`rounded-xl p-[1px] ${GRAD_BG}`}>
            <section className="rounded-xl bg-white">
              ...
              <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-3 p-4">
                <DashLink
                  to="/admin/manage-complaints"
                  icon={<MessageSquareWarning className={`h-5 w-5 ${ICON_COLOR}`} />}
                  label="Manage Complaints"
                />
                <DashLink
                  to="/admin/manage-feedbacks"
                  icon={<ThumbsUp className={`h-5 w-5 ${ICON_COLOR}`} />}
