// src/pages/dashboards/TourPackageManagerDashboard.jsx
...
export default function TourPackageManagerDashboard() {
  ...
  useEffect(() => {
    const t = setTimeout(() => {
      setRecent([
        { id: "TP-2207", title: "Created: ‘Cultural Triangle 5D/4N’", when: "1h ago" },
        { id: "TP-2198", title: "Updated itinerary: ‘South Coast Escape’", when: "Today" },
        { id: "TP-2180", title: "Published promo: ‘Hill Country Highlights’", when: "Yesterday" },
      ]);
      setLoading(false);
    }, 450);
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
      ...
      <div className="mb-6 flex items-center justify-between">
        <h1 className="text-2xl font-semibold tracking-tight">Tour Package Manager</h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <aside className="lg:col-span-1 space-y-4">
            ...
            <section className="rounded-xl bg-white p-4 shadow-sm">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <div className="font-semibold text-neutral-900">Tour Package Manager</div>
                </div>
              </div>

              <div className="mt-4 grid grid-cols-3 gap-2 text-center">
                <MiniStat k="Packages" v="128" />
                <MiniStat k="Active" v="92" />
                <MiniStat k="Drafts" v="14" />
              </div>
          ...
          <NavGroup title="Manage">
            <RailLink
              ...
              icon={<Map className={`h-4 w-4 ${ICON_COLOR}`} />}
            >
              Tour Packages
            </RailLink>
          </NavGroup>
        ...
        <main className="lg:col-span-2 space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <StatCard title="Upcoming departures" value="36" icon={<CalendarDays className={`h-5 w-5 ${ICON_COLOR}`} />} />
            <StatCard title="Active packages" value="92" icon={<Map className={`h-5 w-5 ${ICON_COLOR}`} />} />
            <StatCard title="Draft in review" value="7" icon={<Map className={`h-5 w-5 ${ICON_COLOR}`} />} />
          </div>
          ...
          <div className={`rounded-xl p-[1px] ${GRAD_BG}`}>
            <section className="rounded-xl bg-white">
              ...
              <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-3 p-4">
                <DashLink
                  to="/admin/tour-packages"
                  icon={<Map className={`h-5 w-5 ${ICON_COLOR}`} />}
                  label="Manage Tour Packages"
                />
                {/* Add more *tour-package-only* actions here later if needed */}
              </div>
            </section>
          </div>
...
/* ---------- UI helpers (same look as AdminDashboard) ---------- */
